...
/**
 * Containerd-bridge based implementation of a LogStore.
 *
 * Relies on containerd-bridge's implementation details with regards to the JSON log-driver. When using the JSON log-driver
 * Containerd-bridge writes stdout/stderr to a plain text file which is read by this store. Logs are written in the
 ...
 */
class ContainerdToActivationFileLogStore(system: ActorSystem, destinationDirectory: Path = Paths.get("logs"))
    extends ContainerdToActivationLogStore(system) {
  ...
  override def collectLogs(transid: TransactionId,
    ...
    val toSeq = Flow[ByteString].via(ContainerdToActivationLogStore.toFormattedString).toMat(Sink.seq[String])(Keep.right)
    ...
    val combined = CDOwSink.combine(toSeq, toFile)(Broadcast[ByteString](_))
...
object ContainerdToActivationFileLogStoreProvider extends LogStoreProvider {
  override def instance(actorSystem: ActorSystem): LogStore = new ContainerdToActivationFileLogStore(actorSystem)
}
