...
object LoggingMarkers {
  ...
  def INVOKER_CONTAINERD_CMD(cmd: String) = LogMarkerToken(invoker, "ctr", start, Some(cmd), Map("cmd" -> cmd))
  def INVOKER_CNI_CMD(cmd: String) = LogMarkerToken(invoker, "cni", start, Some(cmd), Map("cmd" -> cmd))
