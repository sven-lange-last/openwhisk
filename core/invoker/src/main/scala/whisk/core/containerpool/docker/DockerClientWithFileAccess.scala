...
class DockerClientWithFileAccess(
    ...
    protected def containerDirectory(containerId: ContainerId) =
        new java.io.File(containersDirectory, containerId.asString).getCanonicalFile()
    ...
    protected def containerConfigFile(containerId: ContainerId) = {
        new java.io.File(containerDirectory(containerId), "config.v2.json").getCanonicalFile()
    }
    ...
    protected def containerLogFile(containerId: ContainerId) =
        new java.io.File(containerDirectory(containerId), s"${containerId.asString}-json.log").getCanonicalFile()
